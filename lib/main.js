...
const presentationConfig = directoriesPaths.map(async (directoryPath) => {
    ...
    const cssThemeName = cliOptions.theme 
                            ...
                            : undefined;
    const cssDefaultFallbackPath = `${currentLibraryPath}/presentationUtil/css/default.css`;

    const presentationHTMLString = await preparePresentation(mdContent, filenames, cssFilePathsInDirectory, cliOptions.css, cssThemeName, cssDefaultFallbackPath);
