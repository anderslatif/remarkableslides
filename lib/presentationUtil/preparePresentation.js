import lookupUniqueCSSStyles from "./prepareCSS.js";

export function preparePresentation(mdContent, filenames) {
    const foundCustomStyles = filenames.map((filename) => lookupUniqueCSSStyles(filename)).join("\n");
    
    const presentableTitle = filenames.map((filename) => filename.replace(".md", "")).join(" || ");

    return presentationHeader
        .replace("$$CUSTOM_STYLES$$", foundCustomStyles ?? "")
...
// I know, I know... but it is much more optimized to keep them as strings rather than reading them from a file every time

const presentationHeader = `<!DOCTYPE html>
<html>
<head>
    <style>
        body {
...
const presentationFooter = `</textarea>
